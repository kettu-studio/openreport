...
export const load: PageServerLoad = async ({ params }) => {
  ...
  // Preparar los tres tipos si existen entre los reportes
  let latestTrivy: any = null;
  let latestGitleaks: any = null;
  let latestSyft: any = null;
  for (const r of reports) {
    ...
    if (!latestSyft && t === 'syft') latestSyft = { id: r.id, prepared: prepareSyftData(json) };
    if (latestTrivy && latestGitleaks && latestSyft) break;
  }
  return { project, reports, latest, latestTrivy, latestGitleaks, latestSyft };
};
